...
On remarque que l’incertitude sur la position varie de façon linéaire entre environ 5.5 et 15.5 nm. On remarque également que le terme dominant dans le calcul d'incertitude, 
...
globale de la position sur le spectromètre. 
...
être sous-estimée. 
%Incertitude sur la résolution comme étant la plus petite division de l'échelle en lambda
%Image de la détermination de FWHM avec un zoom sur le bleu, avec un peu de flat des deux côtés
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\linewidth]{FWHM_bleu_3D.png}
  \caption{Données expérimentales du coefficient de transmission en nuage de points avec barres d'erreurs en fonction de l'angle d'orientation de \(P_2\) par rapport à \(P_1\) et de \(P_3\) par rapport à \(P_2\) dans le système à 3 polariseurs, ainsi que la fonction théorique associée (\(T_3(\theta)\)).}
  \label{FWHM_bleu}
\end{figure}

Les résultats des résolutions obtenus sont présentés au tableau \ref{res_tab}.
...
La résolution obtenue pour le spectromètre sur table optique, de $1,48 \pm 0,4$ est meilleure que celle 
obtenue pour le spectromètre imprimé en 3D, de $1,94 \pm 0,4$. Cela s'explique par 

%Résultat rouge à Mentionner
...
% # positionnement des lentilles à leur focale, aberration des lentilles,
...
% # Incertitudes respectives et dire si elles sont bien/logique ou pas: position: Relativement pas si grand, mais pourrait être amélioré
% # en étalonnant le laser bleu qui sert de référence pour notre spectro, résolution : Bien, assez petit, donner le relatif aussi.
