...
est orientée à 
$29,7{^\circ}$ de l'axe venant de la première lentille et diffractant la lumière à 
...
qui a un pas $\Lambda=600mm^{-1}$ forme un angle de $29,7{^\circ}$ entre sa normale et
...
\end{figure}
 ...
 60 longueurs d'onde différentes variant du rouge (780nm) au bleu (380nm).On rmarque que la graduation de l'axe des adscisse
 traduit la position du capteur par rapport a l'ordre 0. Par conséquant, le capteur a une largeur de 6.656 mm.   
 \begin{figure}[H]
    \centering
    \includegraphics[scale=0.45]{simu2.png}
    \caption{Image simulée sur le capeteur de la caméra }
    \label{arc-en-ciel}
\end{figure}

L'angle d'incidence sur le réseau de diffraction obtenu avec l'équation (\ref{GratingEq})
est $29,7{^\circ}$. Les rayons rouge et bleus diffractés ont un angle de sortie par rapport
à la normale du réseau
...
de positionner le réseau de diffraction à un angle particulier. L'angle est de $90{^\circ}$ entre 
...
plus élevé se retrouvent plus à gauche et inversement pour les longueurs d'onde plus élevées.
...
est donc choisie. La largeur de la fente $a$ n'étant pas limitée, on peut minimiser sa valeur pour améliorer la résolution. Sachant que la fente sera
placée manuellement, on considère que $a$ sera le plus petit possible, tout en laissant le faisceau passer.
...
la diffraction avec la plus forte intensité lumineuse, les graduation en x dans la simulation traduise donc la position du capteur par rapport à l'ordre 0. On remarque également qu'avec les paramètres utilisés, le spectre est plus étroit
